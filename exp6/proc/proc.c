...
static struct proc* alloc_process(void) {
    ...
    for (struct proc *p = proc; p < &proc[NPROC]; p++) {
        if (p->state == UNUSED) {
            ...
            p->wait_start = 0;
            /* 系统调用相关初始化 */
            p->trapframe = (struct trapframe*)(p->kstack + KSTACK_SIZE - sizeof(struct trapframe));
            memset_zero(p->trapframe, sizeof(struct trapframe));
            p->sz = 0;
            return p;
        }
    }
...
int fork(void) {
    ...
    if (p->trapframe && np->trapframe) {
        ...
        char *dst = (char*)np->trapframe;
        for (int i = 0; i < (int)sizeof(struct trapframe); i++)
            dst[i] = src[i];
    }
