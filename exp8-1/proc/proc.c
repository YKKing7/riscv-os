...
void set_priority(struct proc *p, int prio) {
    if (p && prio >= PRIO_MIN && prio <= PRIO_MAX) {
        p->priority = prio;
        p->base_priority = prio;
        p->wait_ticks = 0;  /* 重置等待计数 */
    }
}
...
static void run_proc(struct cpu *c, struct proc *p) {
    ...
    /* 优先级调度: 运行后恢复基础优先级 */
    if (current_scheduler == SCHED_PRIO && p->priority != p->base_priority) {
